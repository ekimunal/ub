...
//filter_characters("Weasley")>>“Ron Weasley”, “George Weasley”, “Ginny Weasley” etc
function filter_characters($filter)
{
    $curl = curl_init();
    ...
    if ($err) {
        ...
        $characters = $response['$name'];
        
        $names = array();
        
        foreach ($characters as $char => $resource) {
            if (strpos($characters['name'], $filter) !== false) {
                echo 'true';
                array_push($names, $$characters['name']);
            }
        }
        return $names;
        //sends found names with filtered string in it
    }
    return true;
}
